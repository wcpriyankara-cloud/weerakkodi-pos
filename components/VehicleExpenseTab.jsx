...
// components/VehicleExpenseTab.jsx
...
const compressImage = (file, maxW = 400, quality = 0.6) =>
  new Promise((resolve) => {
...
const TRANSLATIONS = {
  si: {
    ...
    addItem:             'භාණ්ඩයක්',
    itemsAutoNote:       'භාණ්ඩ එකතු කළොත් මුළු මුදල ස්වයංක්‍රීයව ගණනය වේ.',
    ...
    noExpenses:          'වාහන වියදම් නොමැත',
    clearFilters:        'පෙරහන් ඉවත්',
    today:               'අද',
    thisWeek:            'සතිය',
    thisMonth:           'මාසය',
    lastMonth:           'පසු මාසය',
    ...
    byType:              'වර්ග අනුව',
    byVehicle:           'වාහන අනුව',
    byPayment:           'ගෙවීම් අනුව',
    avgKmL:              'සාමාන්‍ය KM/L',
    fuelEfficiency:      'Fuel efficiency',
    previous:            'පෙර',
    next:                'ඊළඟ',
    categoryManage:      'වර්ග කළමනාකරණය',
    newCategoryName:     'නව වර්ග නම',
    deleteCategoryConfirm:'මෙම වර්ගය මකන්නද?',
    categoryAdded:       'වර්ගය එක් විය!',
    categoryDeleted:     'වර්ගය මකා දැමිණි!',
    supplierPicker:      'සැපයුම්කරු තෝරන්න',
    searchSupplier:      'සැපයුම්කරු සොයන්න...',
    newSupplier:         'නව සැපයුම්කරු',
    supplierName:        'සැපයුම්කරු නම',
    companyName:         'ආයතනයේ නම',
    phone:               'දුරකථන',
    saveSupplier:        'සැපයුම්කරු සුරකින්න',
    ...
    searchItems:         'භාණ්ඩය සොයන්න...',
    noMatchingItems:     'ගැලපෙන භාණ්ඩ නොමැත',
    noItems:             'භාණ්ඩ නොමැත',
    orTypeManually:      'හෝ අතින් ටයිප් කරන්න',
    saved:               'සුරකින ලදී!',
    updated:             'යාවත්කාලීන විය!',
    deleted:             'මකා දමන ලදී!',
    duplicated:          'අනුපිටපත් විය!',
    imageTooLarge:       '5MB ට වඩා විශාලයි',
    imageProcessFailed:  'පින්තූරය process කළ නොහැක',
    selectVehicleError:  'වාහනය තෝරන්න',
    enterAmount:         'මුදල ඇතුළත් කරන්න',
    selectBankError:     'බැංකු ගිණුම තෝරන්න',
    loading:             'පූරණය වෙමින්...',
    manualAmount:        'Manual Amount',
    itemsAutoAmount:     'භාණ්ඩවල එකතුව අනුව මුදල ස්වයංක්‍රීයව සැකසේ',
    fuelTracking:        'Fuel Tracking',
    litres:              'ලීටර',
    pricePerLitre:       'ලීටර මිල',
    kmBefore:            'KM Before',
    kmAfter:             'KM After',
    distanceDriven:      'දාවූ දුර',
    autoTotal:           'Auto Total',
    tyreTracking:        'Tyre Tracking',
    tyreBrand:           'Brand',
    tyrePosition:        'Position',
    installedDate:       'Installed Date',
    installedKm:         'Installed KM',
  ...
  en: {
    ...
    addItem:             'Add Item',
    itemsAutoNote:       'Total auto-calculates when items are added.',
    ...
    noExpenses:          'No vehicle expenses',
    ...
    loading:             'Loading...',
    manualAmount:        'Manual Amount',
    itemsAutoAmount:     'Amount auto-set from items total',
    fuelTracking:        'Fuel Tracking',
    litres:              'Litres',
...
═══════════════════════════════════════ */
const fuelCalc = (f) => {
  const litres     = toNum(f.fuelLitres);
  const kmB        = toNum(f.kmBefore);
  const kmA        = toNum(f.kmAfter);
...
const tyreCalc = (f) => {
  const iKm        = toNum(f.installedKm);
  const cKm        = toNum(f.currentKm);
  ...
  if (selectedItem) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', borderRadius: 10, border: '2px solid #22c55e', background: '#f0fdf4' }}>
        ...
        <button onClick={() => { onSelect(null); setSearch(''); }} style={{ width: 28, height: 28, borderRadius: '50%', border: '1px solid #fecaca', background: '#fef2f2', color: '#dc2626', cursor: 'pointer', fontWeight: 700, fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>X</button>
      </div>
    );
  }
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative' }}>
      <div style={{ position: 'relative' }}>
        <input
          value={open ? search : currentName || ''}
          onChange={(e) => { if (open) setSearch(e.target.value); else onNameChange(e.target.value); }}
          onFocus={() => { setSearch(''); setOpen(true); }}
          placeholder={`🔍 ${t.searchItems}`}
          style={ST.inpSm}
        />
        {!open && (
          <button onClick={() => { setOpen(true); setTimeout(() => inputRef.current?.focus(), 50); }} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: 12 }}>▼</button>
        )}
      </div>
...
export default function VehicleExpenseTab({ vehicles: externalVehicles, lang: externalLang }) {
  ...
  const supplierListFiltered = useMemo(() => {
    ...
    return suppliers.filter((sup) =>
      `${sup.name || ''} ${sup.companyName || ''} ${sup.phone || ''}`.toLowerCase().includes(s)
    );
  ...
  useEffect(() => {
    ...
    if (!externalVehicles) {
      unsubs.push(onSnapshot(collection(db, `users/${user.uid}/vehicles`), (s) =>
        setVehicles(s.docs.map((d) => ({ id: d.id, ...d.data() })))
      ));
    }

    unsubs.push(onSnapshot(
      collection(db, `users/${user.uid}/vehicleExpenses`),
      (s) => {
    ...
    unsubs.push(onSnapshot(collection(db, `users/${user.uid}/bankAccounts`), (s) =>
      setBankAccounts(s.docs.map((d) => ({ id: d.id, ...d.data() })))
    ));
    ...
    unsubs.push(onSnapshot(
      query(collection(db, 'vehicleExpenseCategories'), where('uid', '==', user.uid)),
      (s) => {
        const custom = s.docs.map((d) => ({ id: d.id, ...d.data(), isCustom: true }));
        setCategories([
          ...DEFAULT_CATEGORIES,
          ...custom.map((c) => ({
            key: c.key || c.name, name: c.name,
            icon: c.icon || '📋', color: c.color || '#64748b',
  ...
  const stats = useMemo(() => {
    ...
    const byType = {};
    filtered.forEach((e) => {
      const tp = e.expenseType || 'other';
      if (!byType[tp]) byType[tp] = { total: 0, count: 0 };
      byType[tp].total += toNum(e.amount);
      byType[tp].count++;
    });
    const byVehicle = {};
    filtered.forEach((e) => {
      ...
      byVehicle[v].total += toNum(e.amount);
      byVehicle[v].count++;
    });
    const byPay = {};
    filtered.forEach((e) => {
  ...
  const clearFilters = () => {
    ...
    setVehicleFilter('all'); setDateFilter('thisMonth');
  ...
  const handleAddCategory = async () => {
    ...
    try {
      await addDoc(collection(db, 'vehicleExpenseCategories'), {
        uid: user.uid, name: newCatName.trim(),
        key: newCatName.trim().toLowerCase().replace(/\s+/g, '_'),
        icon: '📋', color: '#64748b', createdAt: serverTimestamp(),
      });
      setNewCatName('');
  ...
  const handleDeleteCategory = async (id) => {
    ...
    try { await deleteDoc(doc(db, 'vehicleExpenseCategories', id)); showToast(`✅ ${t.categoryDeleted}`); }
    catch (e) { showToast(`❌ ${e.message}`); }
  };
  ...
  const handleCreateSupplier = async () => {
    if (!user) return;
    if (!newSupplier.name.trim()) { showToast(`⚠️ ${t.supplierNameRequired}`); return; }
    ...
    try {
      ...
      setShowSupplierModal(false); setShowNewSupplier(false);
      setNewSupplier({ name: '', companyName: '', phone: '' });
  ...
  /* ── Reset ── */
  const resetForm = () => {
    setForm({ ...INIT_FORM, expenseDate: todayStr(), expenseTime: nowHHMM(), installedDate: todayStr() });
    setEditingId(null); setShowForm(false); setShowReceiptSource(false);
    if (receiptBrowseRef.current) receiptBrowseRef.current.value = '';
    if (receiptCameraRef.current) receiptCameraRef.current.value = '';
  };
  ...
  const handleEdit = (exp) => {
    setForm({
      ...
      expenseItems: Array.isArray(exp.expenseItems)
        ? exp.expenseItems.map((r) => ({ itemId: r.itemId || '', itemName: r.itemName || '', qty: String(r.qty ?? 1), unitPrice: String(r.unitPrice ?? ''), unit: r.unit || '', itemImage: r.itemImage || '' }))
        : [],
    });
    ...
    try {
      ...
      if (!editingId && form.paymentMethod === 'cash') {
        await addDoc(collection(db, `users/${user.uid}/cashTransactions`), {
          ...
          paymentMethod: 'cash',
          description: form.description || `${meta.icon} ${meta.displayLabel} - ${selectedVehicle?.vehicleNo || ''}`,
          date: dateStr, time: timeStr, timestamp: txTs, createdAt: serverTimestamp(),
          uid: user.uid, expenseBy: form.expenseBy || '', expenseBySupplierId: form.expenseBySupplierId || '',
          expenseItems: normalizedItems,
        });
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      try {
        const q2   = query(collection(db, `users/${user.uid}/cashTransactions`), where('expenseId', '==', id), where('source', '==', 'vehicleExpense'));
    ...
    try {
      ...
      if (exp.paymentMethod === 'cash') {
        await addDoc(collection(db, `users/${user.uid}/cashTransactions`), {
          ...
          paymentMethod: 'cash',
          description: `${meta.icon} ${meta.displayLabel} - ${exp.vehicleNo || ''}`,
          date: dateStr, time: timeStr, timestamp: txTs, createdAt: serverTimestamp(), uid: user.uid,
        });
      }
  ...
  const exportCSV = () => {
    ...
    a.download = `vehicle_expenses_${todayStr()}.csv`;
    a.click();
  };
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 8px 80px', fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif', background: '#f8fafc', minHeight: '100vh' }}>
      ...
      {toast && (
        <div style={{ position: 'fixed', top: 20, left: '50%', transform: 'translateX(-50%)', background: '#1e293b', color: '#fff', padding: '12px 24px', borderRadius: 10, zIndex: 9999, fontSize: 15, fontWeight: 600, boxShadow: '0 8px 24px rgba(0,0,0,.2)' }}>
          {toast}
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }} className="noPrint">
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', padding: '10px 14px', background: 'white', borderRadius: 10, border: '1px solid #e2e8f0' }}>
            ...
            <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} style={ST.sel}>
              ...
              {Object.entries(EXP_TYPES).map(([k, m]) => <option key={k} value={k}>{m.icon} {(lang === 'en' ? m.labelEn : m.label).split('/')[0].trim()}</option>)}
            </select>
            ...
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} style={ST.sel}>
              <option value="newest">⬇️ {t.newest}</option>
              <option value="oldest">⬆️ {t.oldest}</option>
      ...
      {mainTab === 'analytics' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {stats.typeBreak.length > 0 && (
            <div style={ST.card}>
              <h3 style={ST.cardH}>📊 {t.byType}</h3>
              {stats.typeBreak.map(([type, data]) => {
                const m   = getExpMeta(type, lang);
                const pct = stats.total > 0 ? (data.total / stats.total * 100) : 0;
                return (
                  <div key={type} style={{ marginBottom: 10 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                      <span style={{ fontSize: 13, fontWeight: 600 }}>{m.icon} {m.displayLabel}</span>
                    ...
                    <div style={{ height: 8, background: '#f1f5f9', borderRadius: 4, overflow: 'hidden' }}>
                      <div style={{ width: `${pct}%`, height: '100%', background: m.color, borderRadius: 6 }} />
                    </div>
                  </div>
                );
              })}
          ...
          {stats.vehicleBreak.length > 0 && (
            <div style={ST.card}>
              <h3 style={ST.cardH}>🚛 {t.byVehicle}</h3>
          ...
          {stats.payBreak.length > 0 && (
            <div style={ST.card}>
              <h3 style={ST.cardH}>💳 {t.byPayment}</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(140px,1fr))', gap: 10 }}>
                {stats.payBreak.map(([pay, data]) => {
                  const pm = PAY_METHODS.find((p) => p.value === pay) || { icon: '💰' };
                  return (
                    <div key={pay} style={{ padding: 14, background: '#f8fafc', borderRadius: 10, border: '1px solid #e2e8f0', textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}>{getPayLabel(pay, lang)}</div>
                  ...
                  <div key={exp.id} style={{ background: 'white', borderRadius: 14, border: '1px solid #e2e8f0', borderLeft: `5px solid ${m.color}`, overflow: 'hidden', marginBottom: 8 }}>
                    <div style={{ padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }} onClick={() => setExpandedId(isExpanded ? null : exp.id)}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <div style={{ fontSize: 11, color: '#94a3b8' }}>{isExpanded ? '🔼' : '🔽'}</div>
                      </div>
                    </div>
                      ...
                      <div style={{ padding: '12px 16px', background: '#f8fafc', borderTop: '1px solid #f1f5f9' }}>
                        ...
                        {exp.expenseType === 'fuel' && (
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 10 }}>
                            {[
                              { l: `⛽ ${t.litres}`, v: `${fmtQ(exp.fuelLitres)} L`, c: '#ea580c' },
                              { l: `📏 KM`, v: `${fmtQ(exp.kmDriven)} KM`, c: '#2563eb' },
                              { l: '🏆 KM/L', v: fmtQ(exp.kmPerLitre), c: '#16a34a' },
                              { l: `💰 ${t.costPerKm}`, v: toNum(exp.kmDriven) > 0 ? `Rs.${fmtC(toNum(exp.amount) / toNum(exp.kmDriven))}` : '-', c: '#7c3aed' },
                            ].map((card) => (
                        ...
                        {exp.expenseType === 'tyre' && (
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 10 }}>
                            {[
                              { l: `🛞 ${t.tyrePosition}`, v: String(exp.tyrePosition || '-'), c: '#2563eb' },
                              { l: `📏 ${t.usedKm}`, v: `${fmtQ(exp.kmUsed)} KM`, c: '#2563eb' },
                              { l: `📅 ${t.days}`, v: String(toNum(exp.daysUsed)), c: '#16a34a' },
                              { l: `🎯 ${t.remaining}`, v: toNum(exp.expectedLifeKm) > 0 ? `${fmtQ(exp.remainingKm)} KM` : '-', c: toNum(exp.remainingKm) <= 5000 ? '#dc2626' : '#7c3aed' },
                            ].map((card) => (
                              <div key={card.l} style={{ background: 'white', padding: '10px 12px', borderRadius: 10, textAlign: 'center', border: '1px solid #e2e8f0' }}>
                                <div style={{ fontSize: 10, color: '#64748b', fontWeight: 700 }}>{card.l}</div>
                        ...
                        {exp.expenseType === 'service' && (
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 10 }}>
                            {[
                              { l: `🔧 ${t.serviceKm}`, v: fmtQ(exp.serviceKm), c: '#7c3aed' },
                              { l: `➡️ ${t.nextServiceKm}`, v: fmtQ(exp.nextServiceKm), c: '#2563eb' },
                              { l: `📏 ${t.remaining}`, v: toNum(exp.nextServiceKm) > 0 && toNum(exp.currentKm) > 0 ? fmtQ(toNum(exp.nextServiceKm) - toNum(exp.currentKm)) : '-', c: '#16a34a' },
                            ].map((card) => (
                              <div key={card.l} style={{ background: 'white', padding: '10px 12px', borderRadius: 10, textAlign: 'center', border: '1px solid #e2e8f0' }}>
                                <div style={{ fontSize: 10, color: '#64748b', fontWeight: 700 }}>{card.l}</div>
                                <div style={{ fontSize: 15, fontWeight: 900, color: card.c, marginTop: 4 }}>{card.v}</div>
                                ...
                                return (
                                  <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, background: 'white', border: '1px solid #e2e8f0' }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                      ...
                                      <div style={{ fontSize: 12, color: '#64748b' }}>{toNum(row.qty || 1)} x Rs.{fmtC(row.unitPrice)}</div>
                                    </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', backdropFilter: 'blur(4px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'white', borderRadius: 18, width: '100%', maxWidth: 760, maxHeight: '94vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 22px', background: 'linear-gradient(135deg,#dc2626,#ef4444)', color: 'white' }}>
              ...
              <button onClick={resetForm} style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,.2)', border: 'none', fontSize: 18, cursor: 'pointer', color: 'white' }}>✕</button>
            </div>
            ...
            <div style={{ padding: 22, overflowY: 'auto', flex: 1 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 14 }}>
                <div>
                  <label style={ST.formLabel}>🚛 {t.vehicle} *</label>
                ...
                <div>
                  <label style={ST.formLabel}>📅 {t.date}</label>
                  <input type="date" value={form.expenseDate} onChange={(e) => setField('expenseDate', e.target.value)} style={ST.inp} />
                </div>
                <div>
                  <label style={ST.formLabel}>🕐 {t.time}</label>
                  <input type="time" value={form.expenseTime} onChange={(e) => setField('expenseTime', e.target.value)} style={ST.inp} />
                </div>
              </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 6 }}>
                  {Object.entries(EXP_TYPES).map(([k, m]) => (
                    <button key={k} type="button" onClick={() => setField('expenseType', k)} style={{ padding: '10px 6px', borderRadius: 10, cursor: 'pointer', fontWeight: 600, fontSize: 11, textAlign: 'center', border: form.expenseType === k ? `2px solid ${m.color}` : '1px solid #e2e8f0', background: form.expenseType === k ? `${m.color}12` : 'white', color: form.expenseType === k ? m.color : '#475569' }}>
                      ...
                      <div style={{ lineHeight: 1.2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{(lang === 'en' ? m.labelEn : m.label).split('/')[0].trim()}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              {form.expenseType === 'fuel' && (
                <div style={{ background: '#fff7ed', border: '2px solid #fdba74', borderRadius: 14, padding: 16, marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 10, marginBottom: 12 }}>
                    {[['fuelLitres', t.litres], ['pricePerLitre', t.pricePerLitre], ['kmBefore', t.kmBefore], ['kmAfter', t.kmAfter]].map(([field, label]) => (
                      <div key={field}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
                    {[
                      { l: `📏 ${t.distanceDriven}`, v: `${fmtQ(fs.kmDriven)} KM`, c: '#ea580c' },
                      { l: '🏆 KM/L', v: fmtQ(fs.kmPerLitre), c: '#16a34a' },
                      { l: `💰 ${t.autoTotal}`, v: `Rs.${fmtC(fs.autoAmt)}`, c: '#2563eb' },
                    ].map((x) => (
              ...
              {form.expenseType === 'tyre' && (
                <div style={{ background: '#eff6ff', border: '2px solid #93c5fd', borderRadius: 14, padding: 16, marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
                    ...
                    <div>
                      <label style={ST.formLabelSm}>{t.tyrePosition}</label>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 12 }}>
                    {[['installedKm', t.installedKm], ['currentKm', t.currentKm], ['expectedLifeKm', t.expectedLifeKm]].map(([field, label]) => (
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
                    {[
                      { l: `📏 ${t.usedKm}`, v: `${fmtQ(ts.kmUsed)} KM`, c: '#2563eb' },
                      { l: `📅 ${t.days}`, v: String(ts.daysUsed), c: '#16a34a' },
                      { l: `🎯 ${t.remaining}`, v: toNum(form.expectedLifeKm) > 0 ? fmtQ(ts.remainingKm) : '-', c: ts.remainingKm <= 5000 && toNum(form.expectedLifeKm) > 0 ? '#dc2626' : '#7c3aed' },
                    ].map((x) => (
                      <div key={x.l} style={{ background: 'white', borderRadius: 10, padding: 12, textAlign: 'center' }}>
                        <div style={{ fontSize: 11, color: '#64748b', fontWeight: 700 }}>{x.l}</div>
                        <div style={{ fontSize: 18, fontWeight: 900, color: x.c, marginTop: 4 }}>{x.v}</div>
              ...
              {form.expenseType === 'service' && (
                <div style={{ background: '#faf5ff', border: '2px solid #c4b5fd', borderRadius: 14, padding: 16, marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
                    {[['serviceKm', t.serviceKm], ['nextServiceKm', t.nextServiceKm], ['currentKm', t.currentKm]].map(([field, label]) => (
                      <div key={field}><label style={ST.formLabelSm}>{label}</label><input type="number" value={form[field]} onChange={(e) => setField(field, e.target.value)} style={ST.inpSm} /></div>
                    ))}
                  </div>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <input
                  type="number"
                  value={hasItems ? itemsTotal.toFixed(2) : form.amount}
                  onChange={(e) => !hasItems && setField('amount', e.target.value)}
                  style={{ width: '100%', padding: 16, borderRadius: 14, border: '2px solid #ef4444', fontSize: 28, fontWeight: 900, textAlign: 'center', boxSizing: 'border-box', color: '#dc2626', background: hasItems ? '#fef7ed' : '#fef2f2', outline: 'none', opacity: hasItems ? .85 : 1 }}
                  placeholder={form.expenseType === 'fuel' ? `Auto: ${fmtC(fs.autoAmt)}` : '0.00'}
                  readOnly={hasItems}
                />
                <div style={{ fontSize: 11, color: '#64748b', marginTop: 6 }}>
                  {hasItems ? `ℹ️ ${t.itemsAutoAmount}` : form.expenseType === 'fuel' && fs.autoAmt > 0 ? `ℹ️ Auto: ${fmtQ(fs.litres)}L x Rs.${fmtC(form.pricePerLitre)} = Rs.${fmtC(fs.autoAmt)}` : `✍️ ${t.manualAmount}`}
                </div>
              </div>

              {/* Description */}
              <div style={{ marginBottom: 14 }}>
                <label style={ST.formLabel}>📝 {t.description}</label>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {PAY_METHODS.map((pm) => (
                    <button key={pm.value} onClick={() => setForm((p) => ({ ...p, paymentMethod: pm.value, bankAccountId: pm.needsBank ? p.bankAccountId : '' }))} style={{ flex: 1, padding: '10px 6px', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 12, minWidth: 70, border: form.paymentMethod === pm.value ? '2px solid #3b82f6' : '1px solid #e2e8f0', background: form.paymentMethod === pm.value ? '#eff6ff' : 'white', color: form.paymentMethod === pm.value ? '#3b82f6' : '#64748b' }}>
                      {getPayLabel(pm.value, lang)}
                    </button>
                  ))}
                </div>
              </div>
              ...
              {needsBank && (
                <div style={{ marginBottom: 14, padding: 16, borderRadius: 12, background: 'linear-gradient(135deg,#eff6ff,#dbeafe)', border: '2px solid #93c5fd' }}>
                  ...
                  {!activeBanks.length ? (
                    <div style={{ padding: 14, borderRadius: 10, background: '#fef3c7', border: '1px solid #fcd34d', color: '#92400e', fontSize: 13 }}>⚠️ {t.noBankAccounts}</div>
                  ) : (
                      ...
                      {activeBanks.map((acc) => {
                        ...
                        return (
                          <button key={acc.id} onClick={() => setField('bankAccountId', acc.id)} style={{ padding: '12px 14px', borderRadius: 12, cursor: 'pointer', textAlign: 'left', border: isSel ? '2px solid #2563eb' : '1px solid #cbd5e1', background: isSel ? 'linear-gradient(135deg,#2563eb,#3b82f6)' : 'white', color: isSel ? 'white' : '#1e293b' }}>
                            <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 4 }}>🏦 {String((acc.bankName || '').split('(')[0].trim())}</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
                <div>
                  <label style={ST.formLabelSm}>🔖 {t.reference}</label>
                  <input value={form.reference} onChange={(e) => setField('reference', e.target.value)} style={ST.inpSm} />
                </div>
                <div>
                  ...
                  <button type="button" onClick={openSupplierPicker} style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #e2e8f0', background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, cursor: 'pointer', fontSize: 14 }}>
                    <span style={{ color: form.expenseBy ? '#0f172a' : '#94a3b8', fontWeight: form.expenseBy ? 700 : 500 }}>{form.expenseBy || t.selectSupplier}</span>
                    <span style={{ color: '#64748b' }}>▼</span>
                  </button>
                  {form.expenseBy && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
                      <span style={{ padding: '6px 10px', borderRadius: 999, background: '#ecfeff', color: '#155e75', border: '1px solid #a5f3fc', fontSize: 12, fontWeight: 700 }}>👤 {String(form.expenseBy)}</span>
                      <button onClick={handleClearSupplier} style={{ padding: '6px 10px', borderRadius: 999, border: '1px solid #e2e8f0', background: 'white', color: '#64748b', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>✕</button>
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Notes */}
              <div style={{ marginBottom: 14 }}>
                <label style={ST.formLabelSm}>📋 {t.notes}</label>
              ...
              <div style={{ marginBottom: 16, padding: 14, borderRadius: 12, background: '#fafafc', border: '1px solid #e2e8f0' }}>
                ...
                {!(form.expenseItems || []).length ? (
                  <div style={{ padding: 18, borderRadius: 12, border: '1px dashed #cbd5e1', background: 'white', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 12, color: '#64748b' }}>{t.addItemsPlaceholder}</div>
                  </div>
                ) : (
                      ...
                      return (
                        <div key={idx} style={{ padding: 12, borderRadius: 12, background: 'white', border: '1px solid #e2e8f0' }}>
                          ...
                          <div style={{ marginBottom: 8, display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                            <div style={{ flex: 1 }}>
                              <ItemSearchDropdown inventoryItems={inventoryItems} value={row.itemId || ''} currentName={row.itemName || ''} onSelect={(item) => handleSelectInventoryItem(idx, item)} onNameChange={(v) => updateItemRow(idx, { itemName: v })} t={t} />
                            </div>
                            <button onClick={() => removeItemRow(idx)} style={{ width: 40, height: 40, borderRadius: 8, border: '1px solid #fecaca', background: '#fef2f2', color: '#dc2626', cursor: 'pointer', fontWeight: 700, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>✕</button>
                          </div>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                            <div><label style={ST.formLabelSm}>{t.qty}</label><input type="number" min="0" step="0.01" value={row.qty || ''} onChange={(e) => updateItemRow(idx, { qty: e.target.value })} style={ST.inpSm} /></div>
                            <div><label style={ST.formLabelSm}>{t.unitPrice}</label><input type="number" min="0" step="0.01" value={row.unitPrice || ''} onChange={(e) => updateItemRow(idx, { unitPrice: e.target.value })} style={ST.inpSm} /></div>
                            <div><label style={ST.formLabelSm}>{t.lineTotal}</label><div style={{ padding: 10, borderRadius: 8, background: '#f8fafc', border: '1px solid #e2e8f0', fontWeight: 800, textAlign: 'right', fontSize: 13 }}>Rs.{fmtC(toNum(row.qty || 1) * toNum(row.unitPrice))}</div></div>
                          </div>
                          {row.unit && <div style={{ marginTop: 6, fontSize: 11, color: '#64748b' }}>📏 {String(row.unit)}</div>}
                        </div>
                      );
                    })}
                  </div>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                {form.receipt ? (
                  <div style={{ position: 'relative' }}>
                    ...
                    <button onClick={() => { setField('receipt', null); setShowReceiptSource(false); }} style={{ position: 'absolute', top: 6, right: 6, background: 'rgba(239,68,68,.9)', color: 'white', border: 'none', borderRadius: '50%', width: 28, height: 28, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
                  </div>
                ) : (
                  <>
                    ...
                    {showReceiptSource && (
                      <div style={{ marginTop: 10, padding: 14, borderRadius: 12, background: '#f8fafc', border: '2px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: 10 }}>
                        ...
                        <button onClick={() => receiptBrowseRef.current?.click()} style={{ width: '100%', padding: 13, borderRadius: 10, border: '2px solid #3b82f6', background: '#eff6ff', color: '#1d4ed8', fontWeight: 700, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                          <span style={{ fontSize: 22 }}>🖼️</span>{t.gallery}
                        </button>
                        <button onClick={() => receiptCameraRef.current?.click()} style={{ width: '100%', padding: 13, borderRadius: 10, border: '2px solid #16a34a', background: '#f0fdf4', color: '#15803d', fontWeight: 700, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                          <span style={{ fontSize: 22 }}>📷</span>{t.camera}
                        </button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', backdropFilter: 'blur(4px)', zIndex: 10001, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'white', borderRadius: 18, width: '100%', maxWidth: 520, maxHeight: '94vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 22px', background: 'linear-gradient(135deg,#0f766e,#14b8a6)', color: 'white' }}>
              <h3 style={{ margin: 0, fontSize: 17 }}>👤 {t.supplierPicker}</h3>
              <button onClick={() => setShowSupplierModal(false)} style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,.2)', border: 'none', fontSize: 18, cursor: 'pointer', color: 'white' }}>✕</button>
            </div>
            <div style={{ padding: 22, overflowY: 'auto', flex: 1 }}>
                ...
                <div style={{ padding: 14, borderRadius: 12, background: '#f0fdfa', border: '1px solid #99f6e4', marginBottom: 12 }}>
                  <div style={{ display: 'grid', gap: 8 }}>
                    ...
                    <button onClick={handleCreateSupplier} disabled={creatingSupplier} style={{ padding: 14, background: creatingSupplier ? '#94a3b8' : 'linear-gradient(135deg,#0f766e,#14b8a6)', color: 'white', border: 'none', borderRadius: 10, fontWeight: 900, cursor: creatingSupplier ? 'not-allowed' : 'pointer', opacity: creatingSupplier ? .7 : 1 }}>
                      {creatingSupplier ? `⏳ ${t.saving}` : `✅ ${t.saveSupplier}`}
                    </button>
                  </div>
                </div>
              )}
              <div style={{ maxHeight: '50vh', overflowY: 'auto' }}>
                {!supplierListFiltered.length ? (
                  <div style={{ padding: 18, borderRadius: 12, border: '1px dashed #cbd5e1', background: 'white', textAlign: 'center' }}>
                    <div style={{ fontSize: 28 }}>👤</div>
                    <div style={{ fontSize: 13, color: '#64748b' }}>{supplierSearch ? t.noMatchingSuppliers : t.noSuppliers}</div>
                  </div>
                  ...
                  return (
                    <button key={sup.id} onClick={() => handleSelectSupplier(sup)} style={{ width: '100%', padding: '12px 14px', borderRadius: 12, border: `1px solid ${isS ? '#14b8a6' : '#e2e8f0'}`, background: isS ? '#f0fdfa' : 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', textAlign: 'left', cursor: 'pointer', marginBottom: 8 }}>
                      ...
                      {isS && <div style={{ color: '#0f766e', fontWeight: 800 }}>✓</div>}
                    </button>
                  );
                })}
              </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', backdropFilter: 'blur(4px)', zIndex: 10001, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'white', borderRadius: 18, width: '100%', maxWidth: 420, maxHeight: '94vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 22px', background: '#f8fafc', color: '#1e293b' }}>
              <h3 style={{ margin: 0, fontSize: 17 }}>⚙️ {t.categoryManage}</h3>
              <button onClick={() => setShowCategoryModal(false)} style={{ width: 36, height: 36, borderRadius: '50%', background: '#e2e8f0', border: 'none', fontSize: 18, cursor: 'pointer', color: '#64748b' }}>✕</button>
            </div>
            <div style={{ padding: 22, overflowY: 'auto', flex: 1 }}>
                ...
                {categories.map((cat, idx) => (
                  <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 4px', borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      {cat.isCustom && <button onClick={() => handleDeleteCategory(cat.id)} style={{ background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: 6, padding: '4px 8px', fontSize: 12, cursor: 'pointer', fontWeight: 700 }}>🗑️</button>}
                    </div>
                  </div>
                ))}
              </div>
