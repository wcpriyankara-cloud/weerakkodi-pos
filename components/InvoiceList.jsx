...
// ✅ Fixed: react-router-dom removed, Next.js navigation used
// ✅ Fixed: @/shared/firebase-config import
// ✅ Fixed: @/context/UserContext import
...
const TRANSLATIONS = {
  si: {
    title:'ඉන්වොයිස් ලැයිස්තුව', search:'පාරිභෝගිකයා / Invoice ID සොයන්න...',
    invoiceCode:'Invoice Code', balance:'ශේෂය', netAmount:'ශුද්ධ මුදල',
    invoiceValue:'ඉන්වොයිස් වටිනාකම', returnAmount:'ආපසු', totalDiscount:'මුළු වට්ටම',
    billDiscount:'බිල්පත් වට්ටම', exchange:'හුවමාරු', noData:'ඉන්වොයිස් නොමැත',
    loading:'පූරණය වෙමින්...', loadingAll:'ඉන්වොයිස් load වෙමින්...',
    all:'සියල්ල', drafts:'කෙටුම්පත්', completed:'සම්පූර්ණයි', paid:'ගෙවා ඇත',
    unpaid:'නොගෙවූ', partial:'අර්ධ', deleteConfirm:'මෙම ඉන්වොයිසය මකා දැමීමට අවශ්‍යද?',
    deleted:'✅ මකා දමන ලදී!', cashCustomer:'Cash Customer', newSale:'➕ නව විකුණුම',
    today:'අද', thisWeek:'සතිය', thisMonth:'මාසය', allTime:'සමස්ත', days:'දින',
    delete:'මකන්න', viewDetails:'විස්තර', back:'← ආපසු', paymentMethod:'ගෙවීම් ක්‍රමය',
    paidAmount:'ගෙවූ මුදල', items:'භාණ්ඩ', totalSales:'මුළු විකිණීම්',
    totalPaid:'මුළු ගෙවීම්', totalBalance:'මුළු හිඟ', invoiceCount:'ඉන්වොයිස්',
    linkCopied:'Link copied!', noPortalKey:'Portal Key නැත', youSaved:'ඔබ ඉතිරි කළේ',
    totalDebt:'මුළු ණය', previousBalance:'පෙර ශේෂය', thisBill:'මෙම බිල්පත',
    noDebt:'ණය නැත', deposit:'තැන්පතුව', collectPayment:'💰 ගෙවීම',
    paymentCollection:'💰 ගෙවීම් එකතු', paymentAmount:'ගෙවන මුදල (Rs.)',
    paymentNote:'සටහන', paymentMethodLabel:'ගෙවීම් ක්‍රමය',
    paymentSuccess:'✅ ගෙවීම සාර්ථකයි!', paymentError:'❌ දෝෂයකි',
    paymentRequired:'⚠️ මුදල ඇතුළත් කරන්න', savePayment:'✅ සුරකින්න',
    saving:'සුරකිමින්...', cancel:'අවලංගු', invoiceDue:'හිඟ මුදල',
    currentBalance:'වත්මන් ශේෂය', newBalance:'නව ශේෂය', paymentReceipt:'🧾 රිසිට්',
    receiptUploaded:'✅ Upload විය', cash:'මුදල්', card:'කාඩ්', bank:'බැංකු',
    cheque:'චෙක්', other:'වෙනත්', sendBill:'📤 යවන්න', profitBreakdown:'📊 ලාභ',
    totalProfit:'මුළු ලාභය', quotaError:'Firebase Error',
    quotaDesc:'Data load කිරීමේ දෝෂයකි. නැවත උත්සාහ කරන්න.', retry:'🔄 Retry',
    portalLink:'Portal', remark:'සටහන', revenue:'ආදායම', cost:'පිරිවැය',
    qty:'ප්‍රමාණය', noCostData:'Cost data නොමැත', debugTitle:'🔍 Debug',
    lastUpdated:'අවසන් update', loadMore:'Load More',
  },
  en: {
    title:'Invoice List', search:'Search customer / Invoice ID...',
    invoiceCode:'Invoice Code', balance:'Balance', netAmount:'Net Amount',
    invoiceValue:'Invoice Value', returnAmount:'Return', totalDiscount:'Total Discount',
    billDiscount:'Bill Discount', exchange:'Exchange', noData:'No invoices found',
    loading:'Loading...', loadingAll:'Loading invoices...',
    all:'All', drafts:'Drafts', completed:'Completed', paid:'Paid',
    unpaid:'Unpaid', partial:'Partial', deleteConfirm:'Do you want to delete this invoice?',
    deleted:'✅ Deleted!', cashCustomer:'Cash Customer', newSale:'➕ New Sale',
    today:'Today', thisWeek:'Week', thisMonth:'Month', allTime:'All Time', days:'days',
    delete:'Delete', viewDetails:'Details', back:'← Back', paymentMethod:'Payment Method',
    paidAmount:'Paid Amount', items:'Items', totalSales:'Total Sales',
    totalPaid:'Total Paid', totalBalance:'Total Balance', invoiceCount:'Invoices',
    linkCopied:'Link copied!', noPortalKey:'No portal key', youSaved:'You Saved',
    totalDebt:'Total Debt', previousBalance:'Previous Balance', thisBill:'This Bill',
    noDebt:'No Debt', deposit:'Deposit', collectPayment:'💰 Collect Payment',
    paymentCollection:'💰 Payment Collection', paymentAmount:'Payment Amount (Rs.)',
    paymentNote:'Payment Note', paymentMethodLabel:'Payment Method',
    paymentSuccess:'✅ Payment success!', paymentError:'❌ Error',
    paymentRequired:'⚠️ Enter amount', savePayment:'✅ Save',
    saving:'Saving...', cancel:'Cancel', invoiceDue:'Invoice Due',
    currentBalance:'Current Balance', newBalance:'New Balance',
    paymentReceipt:'🧾 Receipt', receiptUploaded:'✅ Uploaded',
    cash:'Cash', card:'Card', bank:'Bank', cheque:'Cheque', other:'Other',
    sendBill:'📤 Send', profitBreakdown:'📊 Profit', totalProfit:'Total Profit',
    quotaError:'Firebase Error', quotaDesc:'Data load error. Please try again.',
    retry:'🔄 Retry', portalLink:'Portal', remark:'Remark', revenue:'Revenue',
    cost:'Cost', qty:'Qty', noCostData:'No cost data', debugTitle:'🔍 Debug',
    lastUpdated:'Last updated', loadMore:'Load More',
  },
};
  ...
  try {
    const slim = list.map((inv) => ({
      ...
      items: (inv.items || []).map((it) => ({
        name: it.name || '', nameSi: it.nameSi || '',
        sinhalaName: it.sinhalaName || '', qty: it.qty || 0,
        lineTotal: it.lineTotal || 0, yourPrice: it.yourPrice || 0,
        sellingPrice: it.sellingPrice || 0, discAmount: it.discAmount || 0,
        costPrice: it.costPrice || 0, avgBatchCost: it.avgBatchCost || 0,
        photoURL: it.photoURL || '', uom: it.uom || '',
        warrantyCode: it.warrantyCode || '', warrantyPeriod: it.warrantyPeriod || '',
      })),
    }));
...
const getCustomerBalanceBreakdown = (customer, invoice) => {
  ...
  return {
    previousBalance, thisInvoiceDue, totalDebt,
    hasDebt:    totalDebt > 0.01,
    hasDeposit: totalDebt < -0.01,
    isZero:     Math.abs(totalDebt) <= 0.01,
  };
};
...
const invoiceProfit = (items) => {
  ...
  items.forEach((i) => {
    const p = itemProfit(i);
    tr += p.rev;
    if (p.hasCost) { tp += p.lp; tc += p.lc; ok = true; }
  });
  ...
  return (
    <div style={{ marginTop: 10 }}>
      <button
        ...
        style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid #e2e8f0', background: show ? '#f0fdf4' : '#f8fafc', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontWeight: 700, fontSize: 12 }}
      >
        <span>📊 {t.profitBreakdown}</span>
        <span style={{ color: s.tp >= 0 ? '#059669' : '#dc2626', fontWeight: 900 }}>
          Rs.{s.tp.toFixed(2)} ({s.mg.toFixed(1)}%)
        </span>
      </button>
      {show && (
          ...
          {items.map((item, i) => {
            ...
            return (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '8px 10px', borderBottom: '1px solid #f1f5f9', alignItems: 'center', background: p.lp !== null && p.lp < 0 ? '#fef2f2' : i % 2 === 0 ? 'white' : '#fafafa' }}>
                ...
                <div style={{ textAlign: 'right', fontSize: 12, fontWeight: 900, color: c }}>
                  {p.lp === null ? '—' : `${p.lp >= 0 ? '' : '-'}${Math.abs(p.lp).toFixed(0)}`}
                </div>
              </div>
            );
          })}
...
function PaymentModal({ invoice, customer, onClose, onSuccess, user, t }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const newPay = invoice.payAmount + amt;
      const newBal = invoice.netAmount - newPay;
      ...
      alert(`${t.paymentError}: ${e.message}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  if (done) return (
    <div style={S.ov} onClick={onClose}>
      <div style={{ ...S.mb, textAlign: 'center', padding: '40px 24px' }} onClick={e => e.stopPropagation()}>
        ...
        <button type="button" onClick={onClose} style={S.greenBtn}>OK ✓</button>
      </div>
    </div>
  );
    ...
    <div style={S.ov} onClick={onClose}>
      <div style={{ ...S.mb, maxHeight: '92vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ background: 'linear-gradient(135deg,#1e40af,#3b82f6)', padding: '18px', borderTopLeftRadius: 20, borderTopRightRadius: 20, color: 'white', position: 'relative' }}>
          <button type="button" onClick={onClose} style={{ position: 'absolute', top: 10, right: 10, background: 'rgba(255,255,255,0.2)', color: 'white', width: 28, height: 28, borderRadius: '50%', border: 'none', cursor: 'pointer', fontWeight: 700 }}>✕</button>
        ...
        <div style={{ padding: '14px 16px' }}>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
            {due > 0 && (
              <button type="button" onClick={() => setAmount(due.toFixed(2))} style={{ padding: '5px 10px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 6, fontSize: 12, fontWeight: 700, color: '#dc2626', cursor: 'pointer' }}>
                🔴 Rs.{due.toFixed(2)}
              </button>
            )}
            {cbal > 0 && (
              <button type="button" onClick={() => setAmount(cbal.toFixed(2))} style={{ padding: '5px 10px', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 6, fontSize: 12, fontWeight: 700, color: '#ea580c', cursor: 'pointer' }}>
                💳 Rs.{cbal.toFixed(2)}
              </button>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
            {methods.map(m => (
              <button
                ...
                style={{ flex: '1 1 70px', padding: '8px 4px', background: method === m.k ? '#dbeafe' : '#f8fafc', border: `2px solid ${method === m.k ? '#3b82f6' : '#e2e8f0'}`, borderRadius: 8, cursor: 'pointer', fontSize: 11, fontWeight: 700, color: method === m.k ? '#1e40af' : '#64748b', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}
              >
          ...
          {preview ? (
            <div style={{ borderRadius: 8, overflow: 'hidden', border: '2px solid #16a34a', marginBottom: 14 }}>
              ...
              <div style={{ background: '#dcfce7', padding: '6px 10px', display: 'flex', justifyContent: 'space-between' }}>
                ...
                <button type="button" onClick={() => { setImg(''); setPreview(''); }} style={{ background: 'none', border: 'none', color: '#dc2626', cursor: 'pointer', fontWeight: 700 }}>✕</button>
              </div>
            </div>
          ) : (
            <button type="button" onClick={() => fileRef.current?.click()} style={{ width: '100%', padding: 14, border: '2px dashed #93c5fd', borderRadius: 8, background: '#eff6ff', cursor: 'pointer', color: '#1e40af', fontSize: 13, fontWeight: 700, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, marginBottom: 14 }}>
          ...
          {toNum(amount) > 0 && (
            <div style={{ background: '#f0fdf4', border: '1.5px solid #bbf7d0', borderRadius: 8, padding: 10, marginBottom: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#475569', marginBottom: 3 }}>
                <span>{t.currentBalance}</span><span>Rs.{cbal.toFixed(2)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#16a34a', fontWeight: 700 }}>
                <span>- Payment</span><span>Rs.{toNum(amount).toFixed(2)}</span>
              </div>
          ...
          <button
            ...
            style={{ width: '100%', padding: 14, background: saving || toNum(amount) <= 0 ? '#9ca3af' : 'linear-gradient(135deg,#16a34a,#15803d)', color: 'white', border: 'none', borderRadius: 10, fontWeight: 800, fontSize: 16, cursor: saving ? 'wait' : 'pointer', marginBottom: 8 }}
          >
            {saving ? `⏳ ${t.saving}` : `${t.savePayment}${toNum(amount) > 0 ? ` — Rs.${toNum(amount).toFixed(2)}` : ''}`}
          </button>
          <button type="button" onClick={onClose} style={{ width: '100%', padding: 10, background: 'none', border: '1px solid #e2e8f0', borderRadius: 8, color: '#64748b', fontWeight: 700, cursor: 'pointer' }}>
            {t.cancel}
          </button>
        </div>
      </div>
...
export default function InvoiceList({ lang: initialLang = 'si' }) {
  ...
  useEffect(() => {
    const syncLang = (e) => {
      ...
      try {
        const s = e.newValue || localStorage.getItem('language');
        if (s === 'si' || s === 'en') setLang(s);
      } catch {}
    };
  ...
  const fetchCustomerById = useCallback(async (customerId) => {
    if (!customerId || customerId === 'CASH_CUSTOMER') return null;
    if (customerCacheRef.current[customerId]) return customerCacheRef.current[customerId];
    try {
      const snap = await getDoc(doc(db, 'customers', customerId));
      if (!snap.exists()) return null;
      const data = { id: snap.id, ...snap.data() };
      customerCacheRef.current[customerId] = data;
      return data;
    } catch (e) {
      console.warn('fetchCustomerById:', e.message);
      return null;
    }
  }, []);
  ...
  useEffect(() => {
    let cancelled = false;
    if (!selected?.customerId || selected.customerId === 'CASH_CUSTOMER') {
      setSelectedCustomerData(null); return;
    }
    fetchCustomerById(selected.customerId).then(c => {
      if (!cancelled) setSelectedCustomerData(c);
    });
    return () => { cancelled = true; };
  }, [selected?.customerId, fetchCustomerById]);

  /* ── Reset visible count on filter change ── */
  useEffect(() => { setVisibleCount(30); }, [search, statusF, dateF]);

  /* ══════════════════════════════════════════════════════════
     FETCH — 3 primary queries, fallback if needed
     ══════════════════════════════════════════════════════════ */
  const fetchInvoices = useCallback(async (forceRefresh = false, showSpinner = true) => {
    if (!user?.uid) { setLoading(false); return; }
    const uid = user.uid;

    if (!forceRefresh) {
      const cached = loadInvoiceCache(uid);
    ...
    if (showSpinner) { setLoading(true); setLoadMsg(t.loadingAll); }
    setFetchErr(false);

    try {
      const seen = new Map(), log = [];

      const primaryPlans = INVOICE_COLLECTIONS.map(col => ({ col, field: 'uid' }));
      const primaryResults = await Promise.allSettled(
        primaryPlans.map(p =>
          getDocs(query(collection(db, p.col), where(p.field, '==', uid)))
        )
      );

      primaryResults.forEach((res, i) => {
        const { col, field } = primaryPlans[i];
        if (res.status === 'rejected') {
          const c = res.reason?.code || '';
          if (!c.includes('permission') && !c.includes('indexes')) log.push(`⚠️ ${col}.${field}: ${c}`);
          return;
        }
        ...
        res.value.docs.forEach(d => {
          const p = d.ref.path;
          if (!seen.has(p)) seen.set(p, normalizeInvoice(d.data(), d.id, d.ref.parent.id, p));
        });
      });

      // Fallback only if 0 results
      if (seen.size === 0) {
        const fallbackFields = ['userId', 'ownerId', 'createdBy'];
        const fbPlans = [];
        INVOICE_COLLECTIONS.forEach(col =>
          fallbackFields.forEach(field => fbPlans.push({ col, field }))
        );
        ...
        fbResults.forEach((res, i) => {
          const { col, field } = fbPlans[i];
          if (res.status !== 'fulfilled' || res.value.empty) return;
          log.push(`✅ fb ${col}.${field}: ${res.value.size}`);
          res.value.docs.forEach(d => {
            const p = d.ref.path;
            if (!seen.has(p)) seen.set(p, normalizeInvoice(d.data(), d.id, d.ref.parent.id, p));
          });
      ...
      const list = Array.from(seen.values())
        .sort((a, b) => getTsMs(b.createdAt) - getTsMs(a.createdAt));
      setDbg(log);
      setInvoices(list);
      setLastFetch(Date.now());
      saveInvoiceCache(uid, list);
    } catch (e) {
      console.error('Fetch error:', e);
      setFetchErr(true);
      const cached = loadInvoiceCache(uid);
      if (cached && cached.length > 0) { setInvoices(cached); setLastFetch(Date.now()); }
    } finally {
  ...
  useEffect(() => {
    ...
    if (cached && cached.length > 0) {
      setInvoices(cached); setLoading(false); setLastFetch(Date.now());
      fetchInvoices(false, false); // background refresh
    } else {
      fetchInvoices();
    }
  }, [authLoading, user?.uid, fetchInvoices]);

  /* ── Portal link ── */
  const portalLink = useCallback(async (inv) => {
    if (typeof window === 'undefined') return null;
    const c = await fetchCustomerById(inv.customerId);
  ...
  /* ── Open payment ── */
  const openPay = useCallback(async (inv, e) => {
    e?.stopPropagation();
    const customer = await fetchCustomerById(inv.customerId);
    setPayModal({ invoice: inv, customer });
  }, [fetchCustomerById]);

  const openOut = useCallback((inv, e) => {
    e?.stopPropagation();
    setOutputInv(inv);
  }, []);

  /* ── Copy portal link ── */
  const handleCopyPortalLink = useCallback(async (inv) => {
    const link = await portalLink(inv);
  ...
  const filtered = useMemo(() => {
    ...
    return invoices.filter(inv => {
      if (term) {
        const txt = [inv.customerName, inv.invoiceNo, inv.invoiceCode, inv.id, inv.customerPhone]
  ...
  const visibleInvoices = useMemo(() => filtered.slice(0, visibleCount), [filtered, visibleCount]);
  ...
  const profits = useMemo(() => {
    const m = {};
    visibleInvoices.forEach(inv => { m[inv._path] = invoiceProfit(inv.items || []); });
    return m;
  }, [visibleInvoices]);
  ...
  if (selected) {
    ...
    return (
      <div style={{ padding: 12, backgroundColor: '#f8fafc', minHeight: '100vh', paddingBottom: 80 }}>
        ...
        {payModal && (
          <PaymentModal
            invoice={payModal.invoice} customer={payModal.customer}
            user={user} t={t}
        ...
        {toast && (
          <div style={{ position: 'fixed', top: 20, left: '50%', transform: 'translateX(-50%)', background: '#334155', color: '#fff', padding: '10px 20px', borderRadius: 8, zIndex: 9999, fontSize: 14 }}>
            {toast}
          </div>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
          <button
            type="button"
            onClick={() => { setSelected(null); setSelectedCustomerData(null); }}
            style={{ background: '#f1f5f9', border: 'none', padding: '8px 14px', borderRadius: 8, fontSize: 15, cursor: 'pointer', fontWeight: 'bold' }}
          >
            {t.back}
          </button>
          <h2 style={{ margin: 0, fontSize: 18, color: '#0f172a', flex: 1 }}>🧾 {selected.invoiceCode}</h2>
          <button type="button" onClick={() => openOut(selected)} style={{ background: '#6366f1', color: 'white', border: 'none', borderRadius: 8, padding: '8px 12px', fontSize: 13, cursor: 'pointer', fontWeight: 800 }}>{t.sendBill}</button>
          <button type="button" onClick={() => handleDelete(selected)} style={S.ab('#ef4444')}>🗑️</button>
        </div>
        ...
        {selected.status !== 'draft' && selected.balance > 0.01 && (
          <button
            ...
            style={{ width: '100%', padding: '13px 18px', marginBottom: 12, background: 'linear-gradient(135deg,#16a34a,#15803d)', color: 'white', border: 'none', borderRadius: 12, fontWeight: 800, fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
          >
            {t.collectPayment}
            <span style={{ background: 'rgba(255,255,255,0.2)', padding: '2px 10px', borderRadius: 6 }}>
              Rs.{selected.balance.toFixed(2)}
            </span>
          </button>
        )}
        ...
        {customer?.portalAccessKey && (
          <div style={{ background: 'linear-gradient(135deg,#059669,#10b981)', borderRadius: 10, padding: 12, marginBottom: 14, color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
            <div style={{ fontSize: 13, fontFamily: 'monospace', wordBreak: 'break-all' }}>
              🔗 {typeof window !== 'undefined' ? `${window.location.origin}/portal/${customer.portalAccessKey}` : ''}
            </div>
            <button type="button" onClick={() => handleCopyPortalLink(selected)} style={S.portalBtn}>📋</button>
          </div>
        )}
        ...
        {selected.items?.length > 0 && (
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 14, fontWeight: 'bold', color: '#334155', marginBottom: 8 }}>
              📦 {t.items} ({selected.items.length})
            </div>
          ...
          <div style={{ background: bal.hasDebt ? '#fef2f2' : '#f0fdf4', border: `2px solid ${bal.hasDebt ? '#fecaca' : '#bbf7d0'}`, borderRadius: 12, padding: 14, marginBottom: 14 }}>
            ...
            {Math.abs(bal.previousBalance) > 0.01 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4, color: '#64748b' }}>
                <span>{t.previousBalance}</span><span>Rs.{bal.previousBalance.toFixed(2)}</span>
              </div>
            )}
            {bal.thisInvoiceDue > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4, color: '#dc2626' }}>
                <span>{t.thisBill}</span><span>Rs.{bal.thisInvoiceDue.toFixed(2)}</span>
              </div>
            )}
    ...
    <div style={{ padding: 10, backgroundColor: '#f8fafc', minHeight: '100vh', paddingBottom: 80 }}>
      ...
      {fetchErr && (
        <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, padding: '12px 14px', marginBottom: 14, display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button type="button" onClick={() => fetchInvoices(true)} style={{ padding: '8px 14px', background: '#dc2626', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 700 }}>
            {t.retry}
          </button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', marginBottom: 8, background: '#f1f5f9', borderRadius: 8, overflow: 'hidden' }}>
        {['today', 'thisWeek', 'thisMonth', 'allTime'].map(f => (
          <button key={f} type="button" onClick={() => setDateF(f)} style={{ flex: 1, padding: '10px 2px', fontSize: 12, fontWeight: 'bold', border: 'none', cursor: 'pointer', background: dateF === f ? '#3b82f6' : 'transparent', color: dateF === f ? 'white' : '#64748b' }}>
            {t[f]}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', marginBottom: 8, background: '#f1f5f9', borderRadius: 8, overflow: 'hidden' }}>
        {[
          ...
          <button key={tab.k} type="button" onClick={() => setStatusF(tab.k)} style={{ flex: 1, padding: '8px 2px', fontSize: 11, fontWeight: 'bold', border: 'none', cursor: 'pointer', background: statusF === tab.k ? 'white' : 'transparent', color: statusF === tab.k ? '#3b82f6' : '#64748b', boxShadow: statusF === tab.k ? '0 1px 4px rgba(0,0,0,0.08)' : 'none', borderRadius: statusF === tab.k ? 6 : 0 }}>
            {tab.l}
          </button>
        ))}
      </div>
      ...
      <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 10, textAlign: 'right' }}>
        {search || statusF !== 'all' || dateF !== 'allTime'
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8', background: 'white', borderRadius: 10, border: '1px solid #e2e8f0' }}>
          ...
          <button type="button" onClick={() => fetchInvoices(true)} style={{ marginTop: 14, padding: '10px 24px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 700 }}>
            🔄 Refresh
          </button>
        </div>
      ) : (
        <>
          {visibleInvoices.map(inv => {
            ...
            const ps  = profits[rk];
            const sav = inv.totalDiscount + inv.billDiscount;
            return (
                ...
                {isExp && (
                  <div style={{ borderTop: '1px solid #e2e8f0', padding: 12, background: '#f8fafc' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: isDue ? 'repeat(4,1fr)' : 'repeat(3,1fr)', gap: 6 }}>
                      <button type="button" onClick={e => { e.stopPropagation(); setSelected(inv); }} style={S.ac('#6366f1')}>
                        <span style={{ fontSize: 18 }}>👁️</span>
                        <span style={{ fontSize: 9 }}>{t.viewDetails}</span>
                      </button>
                      <button type="button" onClick={e => openOut(inv, e)} style={{ ...S.ac('#8b5cf6'), background: 'linear-gradient(135deg,#ede9fe,#ddd6fe)', border: '1.5px solid #8b5cf6' }}>
                        <span style={{ fontSize: 18 }}>📤</span>
                        <span style={{ fontSize: 9, color: '#6d28d9' }}>{t.sendBill}</span>
                      </button>
                      <button type="button" onClick={e => { e.stopPropagation(); handleDelete(inv); }} style={S.ac('#ef4444')}>
                        <span style={{ fontSize: 18 }}>🗑️</span>
                        <span style={{ fontSize: 9 }}>{t.delete}</span>
                      </button>
                      {isDue && (
                        <button type="button" onClick={e => openPay(inv, e)} style={{ ...S.ac('#16a34a'), background: '#f0fdf4', border: '1.5px solid #16a34a' }}>
                          <span style={{ fontSize: 18 }}>💰</span>
                          <span style={{ fontSize: 9 }}>{t.collectPayment}</span>
                        </button>
                      )}
                    </div>
                  </div>
          ...
          {filtered.length > visibleCount && (
            <div style={{ textAlign: 'center', marginTop: 10, marginBottom: 20 }}>
              <button
                ...
                style={{ padding: '12px 24px', background: '#e2e8f0', color: '#334155', border: 'none', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: 14 }}
              >
