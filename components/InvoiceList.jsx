...
// ✅ Optimized: 3 queries (not 12), lazy customer fetch
// ✅ Cache-first: instant load from sessionStorage
// ✅ Limited initial render (30 items)
...
const TRANSLATIONS = {
  ...
  en: {
    ...
    currentBalance:'Current Balance', newBalance:'New Balance', paymentReceipt:'🧾 Receipt',
    receiptUploaded:'✅ Uploaded', cash:'Cash', card:'Card', bank:'Bank',
    cheque:'Cheque', other:'Other', sendBill:'📤 Send', profitBreakdown:'📊 Profit',
    totalProfit:'Total Profit', quotaError:'Firebase Error',
    quotaDesc:'Data load error. Please try again.', retry:'🔄 Retry',
    portalLink:'Portal', remark:'Remark', revenue:'Revenue', cost:'Cost',
    qty:'Qty', noCostData:'No cost data', debugTitle:'🔍 Debug',
  ...
  try {
    const slim = list.map((inv) => ({
      ...
      items: (inv.items || []).map((it) => ({
        name:it.name||'', nameSi:it.nameSi||'', sinhalaName:it.sinhalaName||'',
        qty:it.qty||0, lineTotal:it.lineTotal||0, yourPrice:it.yourPrice||0,
    ...
    sessionStorage.setItem(`invoices_${uid}`, JSON.stringify({ data:slim, ts:Date.now() }));
  } catch {}
};
...
const itemProfit = (item) => {
  const qty=toNum(item.qty||item.quantity||1), sell=sellOf(item), cost=costOf(item);
  const disc=toNum(item.discAmount||item.discount||0);
  const rev=toNum(item.lineTotal)||(sell*qty-disc*qty);
  const lc=cost*qty, lp=cost>0?rev-lc:null, mg=rev>0&&cost>0?(lp/rev)*100:null;
    ...
    <div style={{marginTop:10}}>
      ...
      {show&&(
        <div style={{marginTop:6,background:'white',borderRadius:8,border:'1px solid #e2e8f0',overflow:'hidden'}}>
          ...
          {items.map((item,i)=>{
            const p=itemProfit(item), c=p.lp===null?'#94a3b8':p.lp>=0?'#059669':'#dc2626';
            return (
...
function PaymentModal({ invoice, customer, onClose, onSuccess, user, t }) {
  ...
  const methods=[
    {k:'cash',l:t.cash,i:'💵'},{k:'card',l:t.card,i:'💳'},
    {k:'bank',l:t.bank,i:'🏦'},{k:'cheque',l:t.cheque,i:'📄'},
    {k:'other',l:t.other,i:'💰'},
  ];
...
/* ══════════════════════════════════════════════════════════════
   MAIN COMPONENT — STATE + FETCH + ACTIONS
   ══════════════════════════════════════════════════════════════ */
export default function InvoiceList({ lang: initialLang = 'si' }) {
  const [lang, setLang] = useState(initialLang);
  useEffect(() => {
    const syncLang = () => { try { const s=localStorage.getItem('language'); if(s) setLang(s); } catch {} };
  ...
  useEffect(() => {
    ...
    fetchCustomerById(selected.customerId).then(c => { if (!cancelled) setSelectedCustomerData(c); });
  ...
  /* ════════════════════════════════════════════════════════════
     ✅ OPTIMIZED FETCH — 3 primary queries, fallback if needed
     ════════════════════════════════════════════════════════════ */
  const fetchInvoices = useCallback(async (forceRefresh = false, showSpinner = true) => {
    ...
    try {
      ...
      // ✅ Primary: 3 queries only
      const primaryPlans = INVOICE_COLLECTIONS.map(col => ({ col, field: 'uid' }));
      const primaryResults = await Promise.allSettled(
        primaryPlans.map(p => getDocs(query(collection(db, p.col), where(p.field, '==', uid))))
      );

      primaryResults.forEach((res, i) => {
        const { col, field } = primaryPlans[i];
        if (res.status === 'rejected') { const c = res.reason?.code || ''; if (!c.includes('permission') && !c.includes('indexes')) log.push(`⚠️ ${col}.${field}: ${c}`); return; }
        if (res.value.empty) return;
        log.push(`✅ ${col}.${field}: ${res.value.size}`);
        res.value.docs.forEach(d => { const p = d.ref.path; if (!seen.has(p)) seen.set(p, normalizeInvoice(d.data(), d.id, d.ref.parent.id, p)); });
      });
      ...
      if (seen.size === 0) {
        ...
        const fbPlans = [];
        INVOICE_COLLECTIONS.forEach(col => fallbackFields.forEach(field => fbPlans.push({ col, field })));
        const fbResults = await Promise.allSettled(
          fbPlans.map(p => getDocs(query(collection(db, p.col), where(p.field, '==', uid))))
        );
      ...
      setDbg(log); setInvoices(list); setLastFetch(Date.now());
      saveInvoiceCache(uid, list);
    } catch (e) {
      console.error('Fetch error:', e); setFetchErr(true);
      const cached = loadInvoiceCache(uid);
    ...
    return (
      <div style={{padding:12,backgroundColor:'#f8fafc',minHeight:'100vh',paddingBottom:80}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14,flexWrap:'wrap'}}>
          <button onClick={()=>{setSelected(null);setSelectedCustomerData(null);}} style={{background:'#f1f5f9',border:'none',padding:'8px 14px',borderRadius:8,fontSize:15,cursor:'pointer',fontWeight:'bold'}}>{t.back}</button>
        ...
        <div style={{fontSize:10,color:'#94a3b8',fontFamily:'monospace',background:'#f1f5f9',padding:'4px 8px',borderRadius:4,marginBottom:12,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>📂 {selected._path}</div>
        ...
        {customer?.portalAccessKey&&(
          <div style={{background:'linear-gradient(135deg,#059669,#10b981)',borderRadius:10,padding:12,marginBottom:14,color:'white',display:'flex',justifyContent:'space-between',alignItems:'center',gap:10}}>
            <div style={{fontSize:13,fontFamily:'monospace',wordBreak:'break-all'}}>🔗 {typeof window!=='undefined'?`${window.location.origin}/portal/${customer.portalAccessKey}`:''}</div>
    ...
    <div style={{padding:10,backgroundColor:'#f8fafc',minHeight:'100vh',paddingBottom:80}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12,flexWrap:'wrap',gap:8}}>
        ...
        <div style={{display:'flex',gap:8}}>
          <button onClick={()=>{clearInvoiceCache(user?.uid);fetchInvoices(true);}} style={{padding:'8px 14px',background:'#f1f5f9',color:'#374151',border:'1px solid #e2e8f0',borderRadius:8,cursor:'pointer',fontWeight:700,fontSize:13}}>🔄</button>
          <Link href="/pos" prefetch={false} style={{padding:'8px 16px',background:'#3b82f6',color:'white',borderRadius:8,fontWeight:'bold',fontSize:14,textDecoration:'none',display:'inline-flex',alignItems:'center'}}>{t.newSale}</Link>
        </div>
      </div>
      ...
      {fetchErr&&(
        <div style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:10,padding:'12px 14px',marginBottom:14,display:'flex',gap:10,alignItems:'center'}}>
          ...
          <div style={{flex:1}}><div style={{fontWeight:800,color:'#dc2626'}}>{t.quotaError}</div><div style={{fontSize:12,color:'#991b1b'}}>{t.quotaDesc}</div></div>
      ...
      <div style={{display:'flex',marginBottom:8,background:'#f1f5f9',borderRadius:8,overflow:'hidden'}}>
        {['today','thisWeek','thisMonth','allTime'].map(f=>(
          <button key={f} onClick={()=>setDateF(f)} style={{flex:1,padding:'10px 2px',fontSize:12,fontWeight:'bold',border:'none',cursor:'pointer',background:dateF===f?'#3b82f6':'transparent',color:dateF===f?'white':'#64748b'}}>{t[f]}</button>
        ))}
      </div>

      <div style={{display:'flex',marginBottom:8,background:'#f1f5f9',borderRadius:8,overflow:'hidden'}}>
        {[{k:'all',l:`${t.all} (${invoices.length})`},{k:'completed',l:t.completed},{k:'paid',l:t.paid},{k:'unpaid',l:t.unpaid},{k:'draft',l:t.drafts}].map(tab=>(
          <button key={tab.k} onClick={()=>setStatusF(tab.k)} style={{flex:1,padding:'8px 2px',fontSize:11,fontWeight:'bold',border:'none',cursor:'pointer',background:statusF===tab.k?'white':'transparent',color:statusF===tab.k?'#3b82f6':'#64748b',boxShadow:statusF===tab.k?'0 1px 4px rgba(0,0,0,0.08)':'none',borderRadius:statusF===tab.k?6:0}}>{tab.l}</button>
        ))}
      </div>
