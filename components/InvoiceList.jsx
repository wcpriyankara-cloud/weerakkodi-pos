...
// Next.js App Router compatible version
...
const TRANSLATIONS = {
  si: {
    ...
    unpaid:             'නොගෙවූ',
    partial:            'අර්ධ',
    deleteConfirm:      'මෙම ඉන්වොයිසය මකා දැමීමට අවශ්යද?',
    deleted:            'මකා දමන ලදී!',
    ...
    paymentError:       'දෝෂයකි',
    paymentRequired:    'මුදල ඇතුළත් කරන්න',
...
const getDateStart = (f) => {
  ...
  if (f === 'thisWeek')  {
    const d = now.getDay();
    return new Date(now.getFullYear(), now.getMonth(), now.getDate() - d + (d === 0 ? -6 : 1)).getTime();
  }
...
const payIcon = (m) => ({
  cash: '💵', card: '💳', etransfer: '📱',
  bank_transfer: '🏦', cheque: '📄', credit: '🏦', bank: '🏦',
})[m?.toLowerCase()] || '💰';
...
/* ══════════════════════════════════════════════════════════════
   PROFIT HELPERS
══════════════════════════════════════════════════════════════ */
const costOf = (i) => toNum(
  i.costPrice || i.buyingPrice || i.unitCost ||
  i.cost || i.avgBatchCost || i.purchasePrice || 0
);
const sellOf = (i) => toNum(
  i.yourPrice || i.sellingPrice || i.unitPrice ||
  i.price || i.salePrice || 0
);
...
const invoiceProfit = (items) => {
  if (!Array.isArray(items) || !items.length) {
    return { tp: 0, tr: 0, tc: 0, mg: 0, ok: false };
  }
...
function ProfitTable({ items, t }) {
  ...
  if (!s.ok) return (
    <div style={{
      ...
      borderRadius: 8, fontSize: 12, color: '#92400e', marginTop: 8,
    }}>
      {t.noCostData}
    </div>
  );
      ...
      {show && (
        <div style={{
            ...
            return (
              <div
                ...
                style={{
                  ...
                  alignItems: 'center',
                  background: p.lp !== null && p.lp < 0 ? '#fef2f2' : i % 2 === 0 ? 'white' : '#fafafa',
                }}
              >
                <div>
                  <div style={{
                ...
                <div style={{ textAlign: 'right', fontSize: 11, color: p.hasCost ? '#475569' : '#94a3b8' }}>
                  {p.hasCost ? p.cost.toFixed(0) : '-'}
                </div>
                <div style={{ textAlign: 'right', fontSize: 11, color: '#2563eb', fontWeight: 700 }}>
                  {p.rev.toFixed(0)}
                </div>
          ...
          <div style={{
            display: 'grid',
            gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr',
            padding: '10px', borderTop: '2px solid #e2e8f0',
            background: s.tp >= 0 ? '#f0fdf4' : '#fef2f2',
          }}>
            <div style={{ fontWeight: 800, fontSize: 12, gridColumn: '1/4' }}>
              {t.totalProfit}
            </div>
            <div style={{ textAlign: 'right', fontSize: 12, color: '#dc2626', fontWeight: 700 }}>
              {s.tc.toFixed(0)}
            </div>
            <div style={{
              textAlign: 'right', fontSize: 14, fontWeight: 900,
              color: s.tp >= 0 ? '#059669' : '#dc2626',
            }}>
              Rs.{s.tp.toFixed(0)}
            </div>
          </div>
...
function PaymentModal({ invoice, customer, onClose, onSuccess, user, t }) {
  ...
  const save = async () => {
    ...
    try {
      await addDoc(collection(db, 'customerTransactions'), {
        ...
        amount:             amt,
        method,
        note:               note.trim() || `Payment ${invoice.invoiceCode}`,
        receiptImage:       img         || '',
        type:               'payment',
        status:             'confirmed',
        uid:                user.uid,
  ...
  if (done) return (
    <div style={S.ov} onClick={onClose}>
      <div
        style={{ ...S.mb, textAlign: 'center', padding: '40px 24px' }}
        onClick={(e) => e.stopPropagation()}
      >
        <div style={{ fontSize: 60 }}>✅</div>
        <h3 style={{ color: '#16a34a', margin: '12px 0 8px' }}>{t.paymentSuccess}</h3>
        <div style={{ fontSize: 28, fontWeight: 900, color: '#16a34a', margin: '10px 0 20px' }}>
          Rs. {toNum(amount).toFixed(2)}
        </div>
  ...
  return (
    <div style={S.ov} onClick={onClose}>
      <div
        style={{ ...S.mb, maxHeight: '92vh', overflowY: 'auto' }}
        onClick={(e) => e.stopPropagation()}
      >
        <div style={{
          ...
          <h3 style={{ margin: '4px 0 2px', fontSize: 17, fontWeight: 800 }}>
            {t.paymentCollection}
          </h3>
          <div style={{ fontSize: 12, opacity: 0.85 }}>
            {invoice.invoiceCode} - {invoice.customerName || t.cashCustomer}
          </div>
        </div>
        ...
        <div style={{ padding: '14px 16px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
            <div style={{
              ...
              border: `1.5px solid ${due > 0 ? '#fecaca' : '#bbf7d0'}`,
              borderRadius: 8, padding: 10, textAlign: 'center',
            }}>
              <div style={{ fontSize: 11, color: '#64748b' }}>{t.invoiceDue}</div>
              <div style={{ fontSize: 18, fontWeight: 900, color: due > 0 ? '#dc2626' : '#16a34a' }}>
                Rs.{due.toFixed(2)}
              </div>
            </div>
            <div style={{
              background: cbal > 0 ? '#fef2f2' : '#f0fdf4',
              border: `1.5px solid ${cbal > 0 ? '#fecaca' : '#bbf7d0'}`,
              borderRadius: 8, padding: 10, textAlign: 'center',
            }}>
              <div style={{ fontSize: 11, color: '#64748b' }}>{t.currentBalance}</div>
              <div style={{ fontSize: 18, fontWeight: 900, color: cbal > 0 ? '#dc2626' : '#16a34a' }}>
                Rs.{cbal.toFixed(2)}
              </div>
            </div>
          </div>
          ...
          <div style={{ position: 'relative', marginBottom: 6 }}>
            <span style={{
              ...
              transform: 'translateY(-50%)', fontWeight: 800, color: '#64748b',
            }}>
              Rs.
            </span>
            <input
              ...
              style={{
                ...
                boxSizing: 'border-box', color: '#059669',
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
            {due > 0 && (
              <button
                type="button" onClick={() => setAmount(due.toFixed(2))}
                style={{
            ...
            {cbal > 0 && (
              <button
                type="button" onClick={() => setAmount(cbal.toFixed(2))}
                style={{
                  padding: '5px 10px', background: '#fff7ed',
                  border: '1px solid #fed7aa', borderRadius: 6,
                  fontSize: 12, fontWeight: 700, color: '#ea580c', cursor: 'pointer',
                }}
              >
                Rs.{cbal.toFixed(2)}
              </button>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
            {methods.map((m) => (
              <button
                ...
                style={{
                  flex: '1 1 70px', padding: '8px 4px',
                  background: method === m.k ? '#dbeafe' : '#f8fafc',
                  border: `2px solid ${method === m.k ? '#3b82f6' : '#e2e8f0'}`,
                  borderRadius: 8, cursor: 'pointer', fontSize: 11,
                  fontWeight: 700, color: method === m.k ? '#1e40af' : '#64748b',
          ...
          {preview ? (
            <div style={{ borderRadius: 8, overflow: 'hidden', border: '2px solid #16a34a', marginBottom: 14 }}>
              <img
                ...
                style={{ width: '100%', maxHeight: 160, objectFit: 'contain', background: '#f8fafc', display: 'block' }}
              />
              <div style={{ background: '#dcfce7', padding: '6px 10px', display: 'flex', justifyContent: 'space-between' }}>
                ...
                <button
                  type="button" onClick={() => { setImg(''); setPreview(''); }}
          ...
          <input
            ...
            onChange={handleImg} style={{ display: 'none' }} accept="image/*"
          />
          ...
          {toNum(amount) > 0 && (
            <div style={{
              ...
              <div style={{
                ...
                <span>- Payment</span>
                <span>Rs.{toNum(amount).toFixed(2)}</span>
              </div>
              <div style={{
                display: 'flex', justifyContent: 'space-between',
                fontSize: 15, fontWeight: 900,
                borderTop: '1px solid #bbf7d0', paddingTop: 6, marginTop: 6,
                color: (cbal - toNum(amount)) > 0.01 ? '#dc2626' : '#16a34a',
              }}>
                <span>{t.newBalance}</span>
                <span>Rs.{(cbal - toNum(amount)).toFixed(2)}</span>
              </div>
            </div>
          )}
          ...
          <button
            ...
            style={{
              width: '100%', padding: 14,
              background: saving || toNum(amount) <= 0
                ? '#9ca3af'
                : 'linear-gradient(135deg,#16a34a,#15803d)',
              color: 'white', border: 'none', borderRadius: 10,
              fontWeight: 800, fontSize: 16,
              cursor: saving ? 'wait' : 'pointer', marginBottom: 8,
            }}
          >
            {saving
              ? `${t.saving}`
              : `${t.savePayment}${toNum(amount) > 0 ? ` - Rs.${toNum(amount).toFixed(2)}` : ''}`}
          </button>
...
export default function InvoiceList({ lang: initialLang = 'si' }) {
  ...
  useEffect(() => {
    ...
    const onStorage = (e) => {
      if (e.key === 'language' && (e.newValue === 'si' || e.newValue === 'en')) {
        setLang(e.newValue);
      }
    };
  ...
  const fetchCustomerById = useCallback(async (customerId) => {
    ...
    if (customerCacheRef.current[customerId]) {
      return customerCacheRef.current[customerId];
    }
    try {
    ...
    try {
      ...
      primaryResults.forEach((res, i) => {
        ...
        if (res.status === 'rejected') {
          ...
          if (!c.includes('permission') && !c.includes('indexes')) {
            log.push(`${col}.${field}: ${c}`);
          }
          return;
        }
        ...
        res.value.docs.forEach((d) => {
          const p = d.ref.path;
          if (!seen.has(p)) {
            seen.set(p, normalizeInvoice(d.data(), d.id, d.ref.parent.id, p));
          }
        });
      });
  ...
  const handleCopyPortalLink = useCallback(async (inv) => {
    ...
    if (!c?.portalAccessKey) {
      showToast(t.noPortalKey);
      return;
    }
    if (typeof window !== 'undefined') {
      navigator.clipboard.writeText(
        `${window.location.origin}/portal/${c.portalAccessKey}`
      );
      showToast(t.linkCopied);
    }
  }, [fetchCustomerById, showToast, t.noPortalKey, t.linkCopied]);
  ...
  const filtered = useMemo(() => {
    ...
    return invoices.filter((inv) => {
      if (term) {
        const txt = [
          inv.customerName, inv.invoiceNo,
          inv.invoiceCode, inv.id, inv.customerPhone,
  ...
  const profits = useMemo(() => {
    ...
    visibleInvoices.forEach((inv) => {
      m[inv._path] = invoiceProfit(inv.items || []);
    });
    return m;
  }, [visibleInvoices]);
      ...
      <div style={{ padding: 12, backgroundColor: '#f8fafc', minHeight: '100vh', paddingBottom: 80 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, fontSize: 13 }}>
            ...
            <div style={{ color: getAgeColor(selected.age), fontWeight: 'bold' }}>
              {selected.age} {t.days}
            </div>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderBottom: idx < selected.items.length - 1 ? '1px solid #f1f5f9' : 'none',
                      background: p.lp !== null && p.lp < 0 ? '#fef2f2' : 'white',
                    }}
                  >
                    <img
                      src={item.photoURL || DEFAULT_SVG} alt=""
                      onError={(e) => { e.target.src = DEFAULT_SVG; }}
                      style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: '#64748b' }}>
                        {item.qty} x Rs.{sellOf(item).toFixed(2)}
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <div style={{ fontWeight: 'bold', fontSize: 15, color: '#1e40af' }}>
                        {toNum(item.lineTotal).toFixed(2)}
                      </div>
                      {p.hasCost && (
                        <div style={{
                          fontSize: 11, fontWeight: 800,
                          color: p.lp >= 0 ? '#059669' : '#dc2626',
                        }}>
                          {p.lp >= 0 ? '+' : ''}Rs.{p.lp?.toFixed(2)}
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <div style={{
          ...
          {[
            ...
            { l: t.totalDiscount, v: (i) => i.totalDiscount,  show: (i) => i.totalDiscount > 0,  c: '#ef4444', px: '-' },
            { l: t.billDiscount,  v: (i) => i.billDiscount,   show: (i) => i.billDiscount > 0,   c: '#f59e0b', px: '-' },
            { l: t.exchange,      v: (i) => i.exchangeAmount, show: (i) => i.exchangeAmount > 0, c: '#8b5cf6', px: '-' },
            { l: t.returnAmount,  v: (i) => i.returnAmount,   show: (i) => i.returnAmount > 0,   c: '#dc2626', px: '-' },
          ].filter((r) => r.show(selected)).map((row, i) => (
            <div key={i} style={{
              display: 'flex', justifyContent: 'space-between',
              fontSize: 14, marginBottom: 5, color: row.c || '#475569',
            }}>
              <span>{row.l}</span>
          ...
          <div style={{
            ...
            borderTop: '2px solid #e2e8f0', paddingTop: 10, marginTop: 5,
          }}>
            <span>{t.netAmount}</span>
            <span>Rs. {selected.netAmount.toFixed(2)}</span>
          </div>
        </div>
        ...
        {selected.remarks && (
          <div style={{
            background: 'white', borderRadius: 10,
            padding: 12, border: '1px solid #e2e8f0', marginBottom: 14,
          }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontWeight: 600 }}>
              {payIcon(selected.paymentMethod)} {(selected.paymentMethod || 'cash').toUpperCase()}
            </span>
          </div>
          <div style={{
            display: 'flex', justifyContent: 'space-between',
            ...
            <span style={{ fontWeight: 600, color: '#16a34a' }}>
              Rs. {selected.payAmount.toFixed(2)}
            </span>
          </div>
          <div style={{
            display: 'flex', justifyContent: 'space-between',
            fontSize: 16, fontWeight: 'bold', padding: '10px',
            borderRadius: 8,
            background:  selected.balance <= 0.01 ? '#f0fdf4' : '#fef2f2',
            color:       selected.balance <= 0.01 ? '#16a34a' : '#dc2626',
          }}>
            <span>{t.balance}</span>
            <span>Rs. {selected.balance.toFixed(2)}</span>
          </div>
        </div>

        {bal && (
          <div style={{
            background: bal.hasDebt ? '#fef2f2' : '#f0fdf4',
            border: `2px solid ${bal.hasDebt ? '#fecaca' : '#bbf7d0'}`,
            borderRadius: 12, padding: 14, marginBottom: 14,
          }}>
            ...
            {Math.abs(bal.previousBalance) > 0.01 && (
              <div style={{
                display: 'flex', justifyContent: 'space-between',
                fontSize: 13, marginBottom: 4, color: '#64748b',
              }}>
                <span>{t.previousBalance}</span>
                <span>Rs.{bal.previousBalance.toFixed(2)}</span>
              </div>
            )}
            {bal.thisInvoiceDue > 0 && (
              <div style={{
                display: 'flex', justifyContent: 'space-between',
                fontSize: 13, marginBottom: 4, color: '#dc2626',
              }}>
                <span>{t.thisBill}</span>
                <span>Rs.{bal.thisInvoiceDue.toFixed(2)}</span>
              </div>
            )}
            <div style={{
              borderTop: '1px dashed #e2e8f0', paddingTop: 8, marginTop: 6,
              display: 'flex', justifyContent: 'space-between',
              fontSize: 17, fontWeight: 900,
              color: bal.hasDebt ? '#dc2626' : bal.hasDeposit ? '#16a34a' : '#059669',
            }}>
              <span>
                {bal.hasDebt ? t.totalDebt : bal.hasDeposit ? t.deposit : t.noDebt}
              </span>
              <span>
                {bal.hasDebt    ? `Rs.${bal.totalDebt.toFixed(2)}` :
                 bal.hasDeposit ? `Rs.${Math.abs(bal.totalDebt).toFixed(2)}` : 'OK'}
              </span>
            </div>
          </div>
        )}
  ...
  return (
    <div style={{ padding: 10, backgroundColor: '#f8fafc', minHeight: '100vh', paddingBottom: 80 }}>
      ...
      {dbg.length > 0 && (
        <details style={{
          marginBottom: 10, background: '#fefce8',
          border: '1px solid #fde047', borderRadius: 8, padding: '8px 10px',
        }}>
          <summary style={{ cursor: 'pointer', fontWeight: 700, fontSize: 11, color: '#854d0e' }}>
            {t.debugTitle}: {invoices.length} loaded
          </summary>
          <pre style={{
            fontSize: 10, color: '#713f12', marginTop: 6,
      ...
      {lastFetch > 0 && (
        <div style={{ fontSize: 11, color: '#94a3b8', textAlign: 'right', marginBottom: 8 }}>
          {new Date(lastFetch).toLocaleTimeString()} | {invoices.length}
        </div>
      )}
      ...
      {fetchErr && (
        <div style={{
          ...
          <button
            ...
            style={{
              padding: '8px 14px', background: '#dc2626',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8, marginBottom: 12 }}>
        {[
          { l: `${t.invoiceCount}`, v: stats.n,                        bg: '#eff6ff', bc: '#3b82f6', tc: '#1e40af' },
          ...
          <div key={i} style={{
            background: s.bg, padding: 10,
            borderRadius: 8, borderLeft: `3px solid ${s.bc}`,
          }}>
            <div style={{ fontSize: 12, color: '#64748b' }}>{s.l}</div>
            <div style={{ fontSize: 17, fontWeight: 'bold', color: s.tc }}>{s.v}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        {['today', 'thisWeek', 'thisMonth', 'allTime'].map((f) => (
          <button
            key={f} type="button" onClick={() => setDateF(f)}
            style={{
              flex: 1, padding: '10px 2px', fontSize: 12, fontWeight: 'bold',
              border: 'none', cursor: 'pointer',
              background: dateF === f ? '#3b82f6' : 'transparent',
              color: dateF === f ? 'white' : '#64748b',
            }}
          >
            {t[f]}
          </button>
      ...
      <div style={{
        ...
        {[
          ...
          <button
            key={tab.k} type="button" onClick={() => setStatusF(tab.k)}
            style={{
              flex: 1, padding: '8px 2px', fontSize: 11, fontWeight: 'bold',
              border: 'none', cursor: 'pointer',
              background: statusF === tab.k ? 'white' : 'transparent',
              color: statusF === tab.k ? '#3b82f6' : '#64748b',
              boxShadow: statusF === tab.k ? '0 1px 4px rgba(0,0,0,0.08)' : 'none',
              borderRadius: statusF === tab.k ? 6 : 0,
            }}
          >
            {tab.l}
          </button>
      ...
      {filtered.length === 0 ? (
        <div style={{
          ...
          <button
            type="button" onClick={() => fetchInvoices(true)}
            style={{
                ...
                >
                  <div style={{
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontWeight: 'bold', fontSize: 18, color: '#1e40af' }}>
                        Rs.{inv.netAmount.toFixed(0)}
                      </div>
                      {ps?.ok && (
                        <div style={{ fontSize: 11, fontWeight: 700, color: ps.tp >= 0 ? '#059669' : '#dc2626' }}>
                          Rs.{ps.tp.toFixed(0)}
                        </div>
                      )}
                      <span style={{
                        fontSize: 11, padding: '2px 8px', borderRadius: 10,
                        background: b.bg, color: b.cl, fontWeight: 'bold',
                      }}>
                        {b.lb}
                      </span>
                    </div>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      color: '#64748b', flexWrap: 'wrap', alignItems: 'center',
                    }}>
                      <span>{inv.date}</span>
                      <span>{inv.itemCount} items</span>
                      <span>{payIcon(inv.paymentMethod)}</span>
                      {sav > 0 && (
                        <span style={{ fontSize: 10, color: '#d97706', fontWeight: 'bold' }}>
                          -{sav.toFixed(0)}
                        </span>
                      )}
                      {isDue && (
                ...
                {isExp && (
                  <div style={{ borderTop: '1px solid #e2e8f0', padding: 12, background: '#f8fafc' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: isDue ? 'repeat(4,1fr)' : 'repeat(3,1fr)', gap: 6 }}>
                      ...
                      <button
                        type="button"
                        onClick={(e) => { e.stopPropagation(); handleCopyPortalLink(inv); }}
                        style={S.ac('#10b981')}
                      >
                        <span style={{ fontSize: 18 }}>🔗</span>
                        <span style={{ fontSize: 9 }}>{t.portalLink}</span>
                      </button>
                      {isDue && (
                        <button
                          type="button"
                          onClick={(e) => openPay(inv, e)}
                          style={{ ...S.ac('#16a34a'), background: '#f0fdf4', border: '1.5px solid #16a34a' }}
                        >
                          <span style={{ fontSize: 18 }}>💰</span>
          ...
          {filtered.length > visibleCount && (
            <div style={{ textAlign: 'center', marginTop: 10, marginBottom: 20 }}>
              <button
                ...
                style={{
                  padding: '12px 24px', background: '#e2e8f0',
                  color: '#334155', border: 'none', borderRadius: 10,
...
const S = {
  ab: (bg) => ({
    ...
    borderRadius: 6, width: 34, height: 34, fontSize: 15,
    cursor: 'pointer', display: 'flex',
    alignItems: 'center', justifyContent: 'center',
  }),
  ac: (bg) => ({
    background: 'white', border: `1px solid ${bg}33`,
    borderRadius: 8, padding: '7px 4px', cursor: 'pointer',
    display: 'flex', flexDirection: 'column',
    alignItems: 'center', gap: 2, color: bg, fontWeight: 'bold',
  }),
  ov: {
