...
// app/(dashboard)/vehicle-income/page.jsx
// Vehicle Income Manager — Next.js App Router version
...
const buildTripInvoice = (trip, vehicles = []) => {
  if (!trip) return null;
  const tripId    = trip.id || Date.now().toString(36);
  const invoiceNo = `TRP-${String(tripId).slice(0, 6).toUpperCase()}`;
  const vehicleNo = trip.vehicleName ||
    vehicles.find((v) => v.id === trip.vehicleId)?.vehicleNo || '';
...
const S = {
  ...
  header: {
    ...
    marginBottom: 30, color: 'white', textAlign: 'center',
  },
  tabs: {
    display: 'flex', gap: 12,
    background: 'white', borderRadius: 16,
    padding: 10, marginBottom: 25, overflowX: 'auto',
  },
  card: {
    background: 'white', borderRadius: 20,
    padding: 30, boxShadow: '0 10px 40px rgba(0,0,0,0.1)',
    marginBottom: 25,
  },
  th: {
  ...
  badge: (bg, c) => ({
    ...
    background: bg, color: c, display: 'inline-block',
  }),
};

/* ══════════════════════════════════════════
   useLang HOOK — Next.js safe
══════════════════════════════════════════ */
function useLang() {
  ...
  return { lang: mounted ? lang : 'si', t };
}

/* ══════════════════════════════════════════
   CUSTOMER SEARCH
══════════════════════════════════════════ */
function CustomerSearch({ uid, value, onChange, t }) {
    ...
    return customers
      .filter(
        (c) =>
          ...
          normalizePhone(c.phone).includes(lq.replace(/\D/g, ''))
      )
      .slice(0, 10);
  }, [customers, q]);
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      >
        {selected ? (
          ...
          <span style={{ color: '#94a3b8' }}>{t.searchCustomer}</span>
        )}
      </div>
      ...
      {open && (
        <div style={{
          ...
          <div style={{ maxHeight: 260, overflowY: 'auto' }}>
            <div
              onClick={() => { onChange('guest', null); setOpen(false); setQ(''); }}
              style={{
...
function AddVehicleModal({ basePath, onClose, onSaved, t }) {
  ...
  const vehicleTypes = [
    ...
    { value: 'lorry',  label: `🚚 ${t.lorry  || 'Lorry'}`  },
      ...
      <div style={S.modal}>
        ...
        <div style={{
          ...
          <div style={S.grid2}>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((p) => ({
                    ...p, vehicleNo: e.target.value.toUpperCase(),
                  }))
                }
                style={S.input}
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <button
              onClick={onClose}
              style={{ ...S.btn('#f1f5f9', '#475569'), flex: 1 }}
            >
              {t.cancel}
            </button>
            <button
...
function AddTripModal({ basePath, uid, vehicles, onClose, onSaved, t }) {
  ...
  const addCargoItem = () =>
    setCargoItems((p) => [
      ...p, { goodsName: '', qty: 1, unitPrice: 0, total: 0 },
    ]);
  ...
  const handleCustomerChange = (custId, custData) => {
    if (custId === 'guest' || !custId) {
      setForm((p) => ({
        ...p, customerId: '', customerName: '', customerPhone: '',
      }));
    } else if (custData) {
      setForm((p) => ({
        ...p,
  ...
  const handleSave = async () => {
    if (!form.vehicleId) { alert(`${t.selectVehicle}!`); return; }
    if (!form.fare && cargoItems.length === 0) {
      alert(`${t.fare}!`); return;
    }
    setSaving(true);
    try {
      ...
      const tripData = {
        ...
        tripDate: Timestamp.fromDate(
          new Date(form.tripDate + 'T12:00:00')
        ),
      ...
      const tripRef = await addDoc(
        collection(db, `${basePath}/vehicleTrips`),
        tripData
      );
        ...
        <div style={{
          ...
          <div style={S.grid2}>
            <div>
              ...
              <input
                ...
                value={form.tripDate}
                onChange={(e) =>
                  setForm((p) => ({ ...p, tripDate: e.target.value }))
                }
                style={S.input}
              />
            </div>
          ...
          <div>
            ...
            <input
              ...
              value={form.fare}
              onChange={(e) =>
                setForm((p) => ({ ...p, fare: e.target.value }))
              }
              style={{
                ...S.input, fontSize: 20,
          ...
          <div>
            ...
            {cargoItems.map((ci, idx) => (
              ...
              >
                ...
                />
                <div style={{
                  padding: '12px 10px', background: '#f0fdf4',
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 8, width: 34, height: 44,
                    cursor: 'pointer', color: '#dc2626', fontSize: 16,
                  }}
                >
                  ✕
          ...
          <div style={{
            ...
            padding: 16, border: '2px solid #e2e8f0',
          }}>
            <div style={{
              display: 'flex', justifyContent: 'space-between',
              marginBottom: 6,
            ...
            <div style={{
              display: 'flex', justifyContent: 'space-between',
              padding: '8px 0', borderTop: '2px solid #e2e8f0', marginTop: 6,
            }}>
...
function ReportsTab({ trips, vehicles, t }) {
  ...
  const filtered = useMemo(() => {
    return trips.filter((tr) => {
      ...
      if (fromDate && ms < new Date(fromDate + 'T00:00:00').getTime())
        return false;
      if (toDate && ms > new Date(toDate + 'T23:59:59').getTime())
        return false;
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        {statCards.map((sc, i) => (
          <div key={i} style={{
            ...S.card, padding: 20, marginBottom: 0,
            textAlign: 'center', borderTop: `4px solid ${sc.color}`,
          }}>
            <div style={{ fontSize: 30, marginBottom: 8 }}>{sc.icon}</div>
            <div style={{ fontSize: 13, color: '#64748b', marginBottom: 4 }}>
              {sc.label}
            </div>
            <div style={{ fontSize: 18, fontWeight: 900, color: sc.color }}>
              {sc.value}
            </div>
          </div>
        ))}
      ...
      <div style={S.card}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {[t.date, t.vehicle, t.customer, t.route,
                  t.totalBill, t.totalReceived, t.balanceDue].map((h) => (
            ...
            {filtered.length > 0 && (
              <tfoot>
                <tr style={{ background: '#f8fafc', fontWeight: 800 }}>
                  <td colSpan={4} style={{ ...S.td, textAlign: 'right' }}>
                    {t.summary}:
                  </td>
...
export default function VehicleIncomeManager() {
  ...
  useEffect(() => {
    ...
    const unsubV = onSnapshot(
      collection(db, `${basePath}/vehicles`),
  ...
  const filteredTrips = useMemo(() => {
    return trips.filter((tr) => {
      ...
      if (searchQ) {
        ...
        if (
          !norm(tr.customerName).includes(q) &&
          !norm(tr.vehicleName).includes(q)
        ) return false;
      }
      return true;
    });
    ...
    <div style={S.container}>
          ...
          <div style={S.modal}>
            ...
            <div style={{ padding: 24 }}>
              <pre style={{ fontSize: 12, overflow: 'auto' }}>
                {JSON.stringify(outputInvoice, null, 2)}
              </pre>
              <button
                onClick={() => setOutputInvoice(null)}
                style={{ ...S.btn('#64748b'), marginTop: 16 }}
              >
                {t.cancel}
              </button>
            </div>
      ...
      <div style={S.header}>
        ...
        <p style={{ margin: '8px 0 0', opacity: 0.85, fontSize: 14 }}>
          {vehicles.length} {t.headerSubtitle} •{' '}
          {trips.length} {t.tripCountLabel}
        </p>
      </div>
        ...
        <div style={S.card}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {[t.tripDate, t.vehicle, t.customer,
                    t.fare, t.paidAmount, t.balanceDue, t.actions]
                    ...
                    >
                      ...
                      <td style={S.td}>
                        <div style={{ fontWeight: 600 }}>
                          {tr.customerName || t.guestCustomer}
                        </div>
                        {tr.customerPhone && (
                          <div style={{ fontSize: 11, color: '#64748b' }}>
                            {tr.customerPhone}
        ...
        <div style={S.card}>
          ...
          <div style={{
            ...
            {vehicles.length === 0 ? (
              ...
              const typeEmoji =
                { truck: '🚛', van: '🚐', lorry: '🚚', pickup: '🛻' }
                [v.vehicleType] || '🚗';

              return (
                <div key={v.id} style={{
                  ...S.card, marginBottom: 0,
                  border: '2px solid #e2e8f0', padding: 20,
                }}>
                  <div style={{
                    display: 'flex', alignItems: 'center',
                  ...
                  <div style={{
                    ...
                    <div style={{ textAlign: 'center' }}>
                      ...
                      <div style={{
                        fontWeight: 800, color: '#16a34a', fontSize: 13,
                      }}>
